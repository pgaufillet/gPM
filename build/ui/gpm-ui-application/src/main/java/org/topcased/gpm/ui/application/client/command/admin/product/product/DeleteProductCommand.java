...
public class DeleteProductCommand extends AbstractCommand implements Command {
    /**
     * Create an DeleteProductCommand.
     * 
     * @param pEventBus
     *            The bus event.
     */
    @Inject
    public DeleteProductCommand(final EventBus pEventBus) {
        super(pEventBus);
    }
    ...
    @Override
    public void execute() {
        fireEvent(GlobalEvent.DELETE_PRODUCT.getType(),
                new DeleteProductAction(getCurrentProductId()),
                GlobalEvent.CLOSE_PRODUCT.getType(), new CloseTabAction(
                        getCurrentProductId(), false),
                MESSAGES.confirmationProductDeletion(1));
    }
}
