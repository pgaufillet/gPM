...
/**
 * A command to export a filter on XLS format.
 * 
 * @author tpanuel
 */
public class ExportSheetTableFilterOnXlsCommand extends AbstractCommand
        implements Command {
    /**
     * Create an ExportFilterOnXlsCommand.
     * 
     * @param pEventBus
     *            The bus event.
     */
    @Inject
    public ExportSheetTableFilterOnXlsCommand(final EventBus pEventBus) {
        super(pEventBus);
    }
    ...
    public void execute() {
        fireEvent(GlobalEvent.EXPORT_FILTER.getType(),
                new ExportFilterResultAction(getCurrentProductWorkspaceName(),
                        getCurrentSheetTableFilterId(), ExportFormat.EXCEL,
                        null));
    }
}