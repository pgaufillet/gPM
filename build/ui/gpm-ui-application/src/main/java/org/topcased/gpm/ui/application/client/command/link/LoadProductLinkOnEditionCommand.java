...
/**
 * LoadProductLinkOnEditionCommand
 * 
 * @author nveillet
 */
public class LoadProductLinkOnEditionCommand extends AbstractLoadLinkCommand {
     ...
     */
    @Inject
    public LoadProductLinkOnEditionCommand(EventBus pEventBus) {
        super(pEventBus);
    }
        ...
        if (lLinkGroup.isEmpty()) {
            fireEvent(GlobalEvent.LOAD_PRODUCT_LINKS.getType(),
                    new GetLinksAction(null, lProductPresenter.getTabId(),
                            pLinkTypeName, DisplayMode.EDITION));
        }
        else {
            lLinkGroup.open();
        }
