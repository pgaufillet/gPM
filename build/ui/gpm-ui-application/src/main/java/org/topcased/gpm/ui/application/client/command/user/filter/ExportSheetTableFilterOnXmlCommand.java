...
/**
 * A command to export a filter on XML format.
 * 
 * @author tpanuel
 */
public class ExportSheetTableFilterOnXmlCommand extends AbstractCommand
        implements Command {
    /**
     * Create an ExportFilterOnXmlCommand.
     * 
     * @param pEventBus
     *            The bus event.
     */
    @Inject
    public ExportSheetTableFilterOnXmlCommand(final EventBus pEventBus) {
        super(pEventBus);
    }
    ...
    public void execute() {
        fireEvent(GlobalEvent.EXPORT_FILTER.getType(),
                new ExportFilterResultAction(getCurrentProductWorkspaceName(),
                        getCurrentSheetTableFilterId(), ExportFormat.XML, null));
    }
}