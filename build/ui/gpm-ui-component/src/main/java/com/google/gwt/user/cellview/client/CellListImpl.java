...
public abstract class CellListImpl<T> {
     ...
     * corresponds to the data at pageStart.
     */
    ...
    /**
     * A boolean indicating whether or not the data size has ever been set. If
     * the data size has never been set, then we will always pass it along to
     * the view.
     */
    ...
    /**
     * As an optimization, keep track of the last HTML string that we rendered.
     * If the contents do not change the next time we render, then we don't have
     * to set inner html.
     */
    ...
    /**
     * Set to true when the page start changes, and we need to do a full
     * refresh.
     */
    ...
    /**
     * The command used to refresh or redraw the page. If both are scheduled,
     * the refresh will take priority.
     */
    private final Scheduler.ScheduledCommand refreshCommand =
    ...
    /**
     * A local cache of the currently selected rows. We cannot track selected
     * keys instead because we might end up in an inconsistent state where we
     * render a subset of a list with duplicate values, styling a value in the
     * subset but not styling the duplicate value outside of the subset.
     */
    ...
    public void setData(List<T> values, int valuesStart) {
        ...
        List<T> boundedValues =
                data.subList(boundedStart - pageStart, boundedEnd - pageStart);
        ...
        if (boundedStart == pageStart
            ...
            for (int i = boundedStart; i < boundedEnd; i++) {
                if (toReplace == null) {
                    ...
                    childContainer.replaceChild(container.getChild(0),
                            toReplace);
     ...
     * 
     * @param sb
     *            the {@link StringBuilder} to build into
     * @param values
     ...
     */
    protected abstract void emitHtml(StringBuilder sb, List<T> values,
     ...
     * 
     * @param html
     *            the html to render in the child
     ...
     * 
     * @param redrawOnly
     *            if true, only schedule a redraw
     */
    ...
    private void updateDataAndView() {
        ...
        int expectedLastIndex =
                Math.max(0, Math.min(pageSize, dataSize - pageStart));
