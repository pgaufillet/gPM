...
public abstract class AbstractListViewAdapter<T> implements ProvidesKey<T> {
    ...
    public static class DefaultRange implements Range, Serializable {
        private static final long serialVersionUID = -2192898666419713226L;
     ...
     *            the new size
     * @param exact
     *            true if the size is exact, false if it is a guess
     */
    ...
    protected void updateViewData(ListView<T> view, int start, int length,
        ...
        if (curStart < end && curEnd > start) {
            ...
            List<T> realValues =
                    values.subList(realStart - start, realStart - start
                            + realLength);
