...
/**
 * Tests the method <CODE>sheetExport<CODE> of the Sheet Service for PDF
 * export.
 * 
 * @author nsamson
 */
public class TestSheetPDFExportService extends AbstractBusinessServiceTestCase {
    ...
    public void testNormalCase() {
        ...
        assertTrue("The export method returns a empty PDF Export.",
                lOutputStream.toByteArray().length != 0);
    }

    /**
     * Tests the sheetExport method when the sheet types have confidential
     * access
     */
    public void testConfidentialAccessCase() {
        ...
        try {
            sheetService.exportSheets(lRoleToken, new ByteArrayOutputStream(),
                    Arrays.asList(lSheetIds), SheetExportFormat.PDF);
