/***************************************************************
 * Copyright (c) 2007 AIRBUS FRANCE. All rights reserved. This
 * program and the accompanying materials are made available
 * under the terms of the Lesser Gnu Public License (LGPL) which
 * accompanies this distribution, and is available
 * at http://www.gnu.org/licenses/lgpl.html
 *
 ******************************************************************/
//
// Attention: Generated code! Do not modify by hand!
// Generated by: Enumeration.vsl in andromda-java-cartridge.
//
package org.topcased.gpm.domain.dictionary;

/**
 * @author Atos
 */
public class CategoryAccess implements java.io.Serializable {
    /**
     * 
     */
    private static final long serialVersionUID = 1L;

    /**
     * 
     */
    public static final CategoryAccess PROCESS = new CategoryAccess("PROCESS");

    /**
     * 
     */
    public static final CategoryAccess PRODUCT = new CategoryAccess("PRODUCT");

    /**
     * 
     */
    public static final CategoryAccess USER = new CategoryAccess("USER");

    private java.lang.String value;

    private CategoryAccess(java.lang.String pValue) {
        this.value = pValue;
    }

    /**
     * The default constructor allowing super classes to access it.
     */
    protected CategoryAccess() {
    }

    /**
     * Return the value
     * 
     * @return the value
     * @see java.lang.Object#toString()
     */
    public String toString() {
        return value;
    }

    /**
     * Creates an instance of CategoryAccess from <code>value</code>.
     * 
     * @param pValue
     *            the value to create the CategoryAccess from.
     */
    public static CategoryAccess fromString(java.lang.String pValue) {
        CategoryAccess lTypeValue = (CategoryAccess) VALUES.get(pValue);
        if (lTypeValue == null) {
            throw new IllegalArgumentException("invalid value '" + pValue
                    + "', possible values are: " + staticLiterals);
        }
        return lTypeValue;
    }

    /**
     * Gets the underlying value of this type safe enumeration.
     * 
     * @return the underlying value.
     */
    public java.lang.String getValue() {
        return this.value;
    }

    /**
     * @see java.lang.Comparable#compareTo(java.lang.Object)
     */
    public int compareTo(Object pObject) {
        return (this == pObject) ? 0 : this.getValue().compareTo(
                ((CategoryAccess) pObject).getValue());
    }

    /**
     * Returns an unmodifiable list containing the literals that are known by
     * this enumeration.
     * 
     * @return A List containing the actual literals defined by this
     *         enumeration, this list can not be modified.
     */
    public static java.util.List<String> literals() {
        return staticLiterals;
    }

    /**
     * Returns an unmodifiable list containing the names of the literals that
     * are known by this enumeration.
     * 
     * @return A List containing the actual names of the literals defined by
     *         this enumeration, this list can not be modified.
     */
    public static java.util.List<String> names() {
        return staticNames;
    }

    /**
     * @see java.lang.Object#equals(java.lang.Object)
     */
    public boolean equals(Object pObject) {
        return (this == pObject)
                || (pObject instanceof CategoryAccess && ((CategoryAccess) pObject).getValue().equals(
                        this.getValue()));
    }

    /**
     * Return the hashCode of the getValue()
     * 
     * @return the hashCode of the getValue()
     * @see java.lang.Object#hashCode()
     */
    public int hashCode() {
        return this.getValue().hashCode();
    }

    private static final java.util.Map<String, CategoryAccess> VALUES =
            new java.util.HashMap<String, CategoryAccess>(3, 1);

    private static java.util.List<String> staticLiterals =
            new java.util.ArrayList<String>(3);

    private static java.util.List<String> staticNames =
            new java.util.ArrayList<String>(3);

    /**
     * Initializes the values.
     */
    static {
        VALUES.put(PROCESS.value, PROCESS);
        staticLiterals.add(PROCESS.value);
        staticNames.add("PROCESS");
        VALUES.put(PRODUCT.value, PRODUCT);
        staticLiterals.add(PRODUCT.value);
        staticNames.add("PRODUCT");
        VALUES.put(USER.value, USER);
        staticLiterals.add(USER.value);
        staticNames.add("USER");
        staticLiterals = java.util.Collections.unmodifiableList(staticLiterals);
        staticNames = java.util.Collections.unmodifiableList(staticNames);
    }
}